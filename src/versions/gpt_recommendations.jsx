      ...
      {[
        ...
        <div key={i}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
            <span style={{ color: item.bad ? T.textMuted : T.cta, fontSize: 14, fontWeight: 600 }}>{item.label}</span>
            <span style={{ color: item.bad ? T.textMuted : T.cta, fontSize: 14, fontWeight: 700 }}>{item.display}</span>
          </div>
          <div style={{ background: T.barBg, borderRadius: 99, height: 10, overflow: "hidden" }}>
            <div style={{ height: "100%", borderRadius: 99, background: item.bad ? T.barBadBg : `linear-gradient(90deg,${T.cta},${T.ctaHover})`, width: inView ? `${item.value}%` : "0%", transition: `width 1.2s cubic-bezier(.4,0,.2,1) ${i * 0.25}s` }} />
          </div>
        </div>
      ))}
    </div>
...
export default function App({ variantId = "gpt_recommendations" }) {
  ...
  const T = isDark ? variantThemes.DARK : variantThemes.LIGHT;
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      setStickyCtaVisible(window.scrollY > 380 && window.innerWidth <= 850);
    };
    const onResize = () => {
      setStickyCtaVisible(window.scrollY > 380 && window.innerWidth <= 850);
    };
  ...
  const css = `
    *{box-sizing:border-box;margin:0;}
    ...
    @media(min-width:851px){.sticky-cta-bar{display:none!important;}}
    @media(max-width:850px){.main-content{padding-bottom:calc(100px + env(safe-area-inset-bottom,0px));}}
  ...
  return (
    <ThemeCtx.Provider value={T}>
      ...
      <div style={{ background: T.bg, minHeight: "100vh", fontFamily: "Manrope,system-ui,sans-serif", color: T.textPrimary, transition: "background 0.4s,color 0.4s" }}>
        ...
        {/* NAV */}
        <nav style={{ position: "relative", zIndex: 100, background: T.navBg, backdropFilter: "blur(20px)", borderBottom: `1px solid ${T.surfBorder}`, padding: "0 clamp(16px,4vw,48px)", transition: "box-shadow 0.3s ease", boxShadow: scrolled ? "0 10px 30px rgba(0,0,0,0.35)" : "none" }}>
          ...
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            ...
            <div className="hero-grid" style={{ display: "flex", alignItems: "center", gap: "clamp(16px,2.5vw,30px)" }}>
              ...
              <div className="hero-structure-col" style={{ flex: "1 1 280px", minWidth: 0, display: "flex", flexDirection: "column", alignItems: "center" }}>
                  ...
                  />
                  <div className="tag-cta hero-structure-badge" style={{ marginTop: 16, background: T.ctaDim, border: `1px solid ${T.ctaBorder}`, color: T.cta }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 20 }}>
                <BentoCard accent="none">
                  <div style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 20 }}>
                    ...
                    <div>
                      <div style={{ fontSize: 14, color: T.statLabelColor, marginBottom: 2 }}>SKALA PROBLEMU</div>
                ...
                <BentoCard accent="none">
                  <div style={{ marginBottom: 20 }}>
                    <div style={{ fontSize: 14, color: T.statLabelColor, marginBottom: 4 }}>CZAS ROZWIĄZANIA SPORU</div>
                  ...
                  <p style={{ marginTop: 18, fontSize: 15, color: T.statNote, fontStyle: "italic" }}>* Dane na podstawie statystyk Ministerstwa Sprawiedliwości 2024</p>
                </BentoCard>

                <BentoCard>
                  <div style={{ fontSize: 14, color: T.statLabelColor, marginBottom: 14 }}>DLACZEGO WARTO DZIAŁAĆ TERAZ</div>
                  {[
              ...
              {[
                ...
                <FadeIn key={i} delay={i * 0.12}>
                  <BentoCard accent={card.accent} style={{ height: "100%" }}>
                    ...
                    <p style={{ color: card.iC, fontSize: 14, fontWeight: 600, marginBottom: 14 }}>{card.sub}</p>
                ...
                {!submitted ? (
                  <>
                    ...
                    <div className="form-fields" style={{ display: "flex", flexDirection: "column" }}>
                      <div>
                            <label style={{ display: "block", color: T.textSecondary, fontSize: 14, fontWeight: 600, marginBottom: 6, letterSpacing: ".03em" }}>
                      ...
                      <div>
                        <label style={{ display: "flex", alignItems: "center", gap: 8, color: T.textSecondary, fontSize: 14, fontWeight: 600, marginBottom: 6, letterSpacing: ".03em" }}>
                              NUMER TELEFONU
                        </label>
                        <div style={{ position: "relative" }}>
                      ...
                      <label style={{ display: "flex", alignItems: "flex-start", gap: 12, cursor: "pointer", marginTop: 4 }}>
                            <div
                              role="checkbox"
                              aria-checked={rodoChecked}
                              aria-labelledby="rodo-consent-text"
                              tabIndex={0}
                ...
                <table className="compare-table" style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: T.tableHeadBg }}>
                      <th style={{ padding: "16px 20px", textAlign: "left", color: T.textMuted, fontSize: 14, fontWeight: 600, letterSpacing: ".06em", textTransform: "uppercase", borderBottom: `1px solid ${T.tableHeadBorder}` }}>Aspekt</th>
                      ...
                      <tr key={i} style={{ borderBottom: `1px solid ${T.tableRowBorder}`, background: i % 2 === 0 ? "transparent" : T.tableRowAlt }}>
                        ...
                        <td style={{ padding: "14px 20px", textAlign: "center" }}>
                          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: T.tableBadBg, borderRadius: 8, padding: "4px 10px" }}>
                            ...
                            <span style={{ fontSize: 14, color: T.tableBadColor }}>{bad}</span>
                          </div>
                        </td>
                      </tr>
                    ))}
        ...
        <footer style={{ position: "relative", zIndex: 1, borderTop: `1px solid ${T.footerBorder}`, padding: "32px clamp(16px,4vw,48px)" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
            ...
            <p style={{ color: T.footerText, fontSize: 14 }}>© 2026 Klim Zavadski, firma KZ · info@rentstandard.pl</p>
            <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "center" }}>
              {[
                ...
                <a key={label} href={href} {...(external ? { target: "_blank", rel: "noopener noreferrer" } : {})} style={{ color: T.footerLink, fontSize: 14, textDecoration: "none", transition: "color 0.2s" }}
                  onMouseEnter={e => e.target.style.color = T.cta}
                  onMouseLeave={e => e.target.style.color = T.footerLink}
                >{label}</a>
              ))}
              <CookieFooterButton style={{ fontSize: 14 }} />
            </div>
          </div>
        </footer>
        </main>
