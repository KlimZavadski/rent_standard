...
- **GitHub Pages**: workflow `.github/workflows/deploy-pages.yml` builds the project with `--base=/<repo>/` and deploys the `dist/` output to GitHub Pages on **push to `main`** (or `workflow_dispatch`). The workflow sets `VITE_PUBLIC_SITE_ORIGIN` to the Pages URL so Open Graph / Twitter meta match that host.
- **Vercel**: connect the repo; framework **Vite**, build `npm run build`, output `dist`. **`vercel.json`** adds SPA rewrites and an **`ignoreCommand`** that **only builds the `vercel` branch** (in Git: `origin/vercel` is the same branch; **`VERCEL_GIT_COMMIT_REF` is just `vercel`**). Set **Settings → Git → Production Branch** to **`vercel`**. Merge or push to **`vercel`** when you want a Vercel production deploy; **`main`** and other branches are ignored by Vercel. Set **`VITE_PUBLIC_SITE_ORIGIN`** to your production origin (no trailing slash). Reuse the same **`VITE_*`** env vars as in GitHub Actions where applicable.
